...
public class ProtocolARP extends Protocol {
		...
		while(true) {
			...
			if (protocolARP.arpTable.containsKey(Utils.ipBytesToString(ipAddr))) {
	    		...
	    		if( (System.currentTimeMillis() - 30*1000) < answer.getTimestamp() ) {
	    			...
	    			answerOnCache = true;
		    	    System.out.println("ARP Query App: Answer was already on our table, " + Utils.ipBytesToString(ipAddr)
	...
	public static ARPPacket generateArpResponse(byte[] senderHardwarAdd, byte[] senderProtoAdd,
			byte[] target_hardaddr, byte[] target_protoaddr) {
		byte[] broadcast = new byte[] { (byte) 255, (byte) 255, (byte) 255, (byte) 255, (byte) 255, (byte) 255 };
		ARPPacket arp = new ARPPacket();
		arp.hardtype = ARPPacket.HARDTYPE_ETHER;
		arp.prototype = ARPPacket.PROTOTYPE_IP;
		...
		while (running) {
			...
			if (!packets.isEmpty()) {
				...
				switch(arpPacket.operation){
					...
					default: System.err.println("ProtocolARP: UNKNOWN packet received.");break;
				}
